...
function initializeForm() {
    if (props.defaultDate) {
        const date = props.defaultDate.toISOString().split('T')[0]
        form.startDate = date
        
        if (props.defaultTime !== null && props.defaultTime !== undefined) {
            form.allDay = false
            form.startTime = `${props.defaultTime.toString().padStart(2, '0')}:00`
            
            // 自动设置结束时间为开始时间后1小时
            const endHour = (props.defaultTime + 1) % 24
...
watch(() => form.allDay, (isAllDay) => {
    if (isAllDay) {
        ...
        // 如果有默认时间，使用默认时间，否则使用当前时间
        const now = new Date()
        const hour = props.defaultTime ?? now.getHours()
